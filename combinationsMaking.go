...
func getStringArrFromMask(str []string, mask string) []string {
	var res []string = make([]string, 5)
	cnt := 0
	for i := 0; i < len(mask); i++ {
		if mask[i] == '1' {
			res[cnt] = string(str[i])
			cnt++
		}
	}
	return res
...
func makeCombinationsFromMasks(inputStr []string, k int) [][]string {
	n := len(inputStr)
	combinations := getBinaryCombinations(n, k)
	res := make([][]string, len(combinations))
	cnt := 0
	for i := 0; i < len(combinations); i++ {
		res[cnt] = getStringArrFromMask(inputStr, combinations[i])
		cnt++
	}
	return res
}
